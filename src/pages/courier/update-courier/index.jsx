...
const Index = () => {
  ...
  const courierData = useQuery({
    queryKey: ["getCourierById", id],
    queryFn: CourierService.getById
  })

  useEffect(() => {
    const {isSuccess, data} = courierData;

    if (isSuccess && Object.keys(data).length !== 0) {
      form.reset({
        full_name: courierData.data.full_name && courierData.data.full_name,
        image: courierData.data.image && courierData.data.image,
        phone_number: courierData.data.phone_number
          ? courierData.data.phone_number.slice(-9)
          : "",
        birth_date: courierData.data.birth_date && courierData.data.birth_date,
        gender: courierData.data.gender && courierData.data.gender,
        password: courierData.data.password && courierData.data.password.toString(),
        car_name: courierData.data.car_name && courierData.data.car_name,
        car_number: courierData.data.car_number && courierData.data.car_number,
        address: courierData.data.address && courierData.data.address,
        jshir: courierData.data.jshir && courierData.data.jshir.toString(),
        bag_number: courierData.data.bag_number && courierData.data.bag_number.toString(),
        driver_license: courierData.data.driver_license && courierData.data.driver_license.toString(),
        driver_license_date: courierData.data.driver_license_date && courierData.data.driver_license_date,
      });
    }
  }, [courierData.isSuccess, courierData.data]);

  const mutation = useMutation({
    mutationFn: CourierService.update,
    onError: (error) => {
      console.log(error)
    ...
    <Layout>
      <Layout.Body>
        <div className="mb-2 flex flex-col gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Kuryerni yangilash</h2>
          </div>
          <Form {...form}>
